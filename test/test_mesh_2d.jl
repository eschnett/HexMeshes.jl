# Tests for the 2D `QuadMesh` family — uniform `make_quad_mesh` only.
# Curvilinear families (`make_cubed_square_mesh`,
# `make_inflated_square_mesh`) go in their own file once those builders
# land.

using HexMeshes
using HexMeshes: QuadMesh, make_quad_mesh, make_cubed_square_mesh,
                 make_inflated_square_mesh, InflatedSquareMesh,
                 PatchInfo2D, nv,
...
@testset "mesh (2D)" begin
    ...
    @testset "make_quad_mesh(T, Mx, My, …): shapes" begin
        m = make_quad_mesh(Float64, 3, 4, 0.0, 1.0)
        @test m isa QuadMesh{Float64}
        @test m.Ne == 3 * 4
        @test size(m.neighbour)      == (4, 12)
        @test size(m.neighbour_face) == (4, 12)
        @test size(m.orientation)    == (4, 12)
        @test size(m.bdry)           == (4, 12)
    ...
    @testset "Square shorthand: equal element count in both axes" begin
        m = make_quad_mesh(Float64, 3, 0.0, 1.0)
        @test m.Ne == 9
        @test nv(m) == 16
    end
    ...
    @testset "shared vertices: adjacent elements reuse the same vertex ID" begin
        Mx, My = 3, 3
        m = make_quad_mesh(Float64, Mx, My, 0.0, 1.0)
        @test nv(m) == (Mx + 1) * (My + 1)
    ...
    @testset "orientation is always 0 (axis-aligned)" begin
        m = make_quad_mesh(Float64, 4, 0.0, 1.0)
    ...
    @testset "vertex coordinates: regular grid on [x0, x1]²" begin
        Mx, My = 2, 3
        m = make_quad_mesh(Float64, Mx, My, -1.0, 2.0)
        hx = (2.0 - -1.0) / Mx
        hy = (2.0 - -1.0) / My
    ...
    @testset "invert_element_map: round-trips bilinear_map" begin
        # A non-trivial distorted quad
        verts = (SVector{2, Float64}(0.0, 0.0),
    ...
    @testset "locate_point: returns the element containing the point" begin
        m = make_quad_mesh(Float64, 3, 3, 0.0, 1.0)
    ...
    @testset "make_cubed_square_mesh: shapes + element count" begin
        for (M, R) in ((3, 0.3), (4, 0.5), (5, 0.1))
            ...
            @test size(m.neighbour) == (4, expected_Ne)
        end
    end
    ...
    @testset "make_cubed_square_mesh: neighbour pointers are symmetric" begin
        ...
        for e in 1:m.Ne, f in 1:4
            n = m.neighbour[f, e]
            n == 0 && continue
            f_back = m.neighbour_face[f, e]
            @test m.neighbour[f_back, n] == e
        end
    end
    ...
    @testset "make_inflated_square_mesh: types and shape" begin
        ...
        @test m isa InflatedSquareMesh{Float64}
        @test m.L == 0.1 && m.R1 == 0.3 && m.R2 == 1.0
        @test m.Ne == length(m.patch_info)
        @test size(m.vertex_coords) == (2, nv(m))
        @test size(m.vertex_idx) == (4, m.Ne)
        # Forwarded properties via getproperty.
        @test m.Ne == m.base.Ne
        @test m.bdry === m.base.bdry
    end

    @testset "make_inflated_square_mesh: patch_info kinds" begin
        m = make_inflated_square_mesh(Float64, 0.1, 0.3, 1.0, 4)
        kinds = sort(unique(p.kind for p in m.patch_info))
        # 0 (inner square) + 1..4 (inflation) + 5..8 (shell).
        @test kinds == [HexMeshes.PatchKind2D(i) for i in 0:8]
        @test all(k -> k isa HexMeshes.PatchKind2D, kinds)
    end
    ...
    @testset "make_inflated_square_mesh: outer boundary on the circle |x| = R2" begin
        ...
        for e in 1:m.Ne, f in 1:4
            ...
            for v_local in corner_pairs[f]
                ...
                @test sqrt(x^2 + y^2) ≈ m.R2 atol=1e-12
            end
        end
    end
    ...
    @testset "make_inflated_square_mesh: outer_bc = :sommerfeld" begin
        ...
        @test count(==(Int8(7)), m.bdry) == 4 * 4   # M faces × 4 directions
        @test count(==(Int8(1)), m.bdry) == 0       # no Dirichlet tag
    end
    ...
    @testset "make_inflated_square_mesh: _patch_point_and_jac_2d well-formed" begin
        ...
        for (e, pi) in enumerate(m.patch_info)
            is_cubical(pi.kind) && continue   # skip inner-square (bilinear)
            for η in sample_xi, ξ in sample_xi
                P, J = HexMeshes._patch_point_and_jac_2d(pi, ξ, η, L_, R1_, R2_)
                @test all(isfinite, P)
                @test all(isfinite, J)
                @test det(J) > 0
            end
    ...
    @testset "make_inflated_square_mesh: locate_point forwarded through wrapper" begin
        m = make_inflated_square_mesh(Float64, 0.1, 0.3, 1.0, 4)
    ...
    @testset "make_inflated_square_mesh: global_to_patch returns NaN on wrong-patch query" begin
        ...
        @test isnan(global_to_patch(m, 2, SVector(0.0, -0.5))[1])
        # Point in +x inflation, asked as +y inflation → outside, NaN
        @test isnan(global_to_patch(m, 4, SVector(0.5, 0.0))[1])
        # Inner-square point, asked as a shell patch → outside, NaN
        @test isnan(global_to_patch(m, 6, SVector(0.0, 0.0))[1])
    end
